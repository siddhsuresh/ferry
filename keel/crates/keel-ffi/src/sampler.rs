...
//!
//! Both `UploadFiles` and `DownloadFiles` spawn a poller thread that, every
//! 500 ms, reads a single shared slot and, if it holds an event, marshals + fires
//! it through the appropriate callback. The transfer's own preprocess/progress
//! callbacks only ever *write* that slot (overwriting the previous value); the
//! poller samples it.
//!
...
//!     arrives, the same one is re-emitted each tick;
//!   * **success = stop-poller-then-final-progress-then-done** — stopping blocks
//!     until the poller thread stops, then `SendTransferFilesDone` fires. The
//!     latest progress snapshot is re-emitted after the join so a sub-500 ms
//!     transfer still exposes its terminal counters;
//!   * **error = done-then-stop** — `SendError` precedes the stop, so one stale
//!     progress event may still slip out after the error; Swift tolerates it.
//!
//! The stop signal is an `AtomicBool` + `JoinHandle`: [`Sampler::stop`] sets the
//! flag and joins (the "blocks until stopped" guarantee), and [`Drop`] does the
//! same so a panic unwinding through a transfer (abi.rs's `catch_unwind`) never
//! orphans the poller thread.
//!
//! Each write stores a **clone** of the progress value, so the poller always reads
//! a consistent snapshot rather than racing a half-updated struct the transfer
//! thread is mutating.
...
/// The poller's sleep between ticks.
const TICK: Duration = Duration::from_millis(500);

/// The single shared slot's payload — one of an upload/download preprocess event
/// or a progress event. Cloned into the slot on every write so the poller reads a
/// consistent snapshot (see the module doc).
#[derive(Clone)]
pub(crate) enum Sample {
    /// Upload preprocess event → SendUploadFilesPreprocess. keel-vfs's upload
    /// preprocess callback yields `(&Metadata, full_path)`; abi.rs derives `name`
    /// (the path base) and `size` here.
    UploadPreprocess {
        full_path: String,
        name: String,
        size: i64,
    },
    /// Download preprocess event → SendDownloadFilesPreprocess.
    DownloadPreprocess {
        full_path: String,
        name: String,
        size: i64,
    },
    /// Progress event → SendTransferFilesProgress. Boxed so this (much larger)
    /// variant doesn't bloat every slot write; a progress tick already clones the
    /// `ProgressInfo`, so the extra box allocation is noise against the USB-bound
    /// transfer.
...
impl Sampler {
    ...
    pub(crate) fn start(on_preprocess: OnCbResult, on_progress: OnCbResult) -> Sampler {
        ...
        let handle = thread::spawn(move || {
            // The stop signal is checked FIRST each iteration.
            loop {
    ...
    /// abi.rs's transfer callbacks.
    pub(crate) fn set(&self, sample: Sample) {
        let mut guard = self.slot.lock().unwrap_or_else(PoisonError::into_inner);
        *guard = Some(sample);
    }

    /// Stop the poller and wait for it to finish — blocks until the poller thread
    /// has stopped. The join waits at most one `TICK` (the current sleep) to
    /// elapse, which is the ≤ 500 ms "done may lag" window on the success path.
...
/// Marshal one sample and fire it through the matching callback. Upload/Download
